...
function open(filePath) {
	...
	const readConf = new Promise((fullfill, reject) => {
		fs.readFile(
			path.join(path.dirname(file), "configuration.json"),
			"utf8",
			(err, data) => {
				if (err) {
	...
	const readRemedi = new Promise((fullfill, reject) => {
		fs.readFile(
			path.join(path.dirname(file), "remedi.json"),
			"utf8",
			(err, data) => {
				if (err) {
	...
	const readCustomCss = new Promise((fullfill, reject) => {
		fs.readFile(
			path.join(path.dirname(file), "css", "custom.css"),
			"utf8",
			(err, data) => {
				if (err) {
					fullfill({ customCss: "" });
	...
	const readFile = new Promise((fullfill, reject) => {
		fs.readFile(file, "utf8", (err, data) => {
			if (err) {
				...
				let dataWithReplacedPaths = replaceRelativeMediaPathWithAbsoluteInMd(
					data,
					absolutePath
				);
...
function save(options) {
	...
	return new Promise((fullfill, reject) => {
		...
		if (withDependecies) {
			...
			if (
				conf.parallaxBackgroundImage &&
				conf.parallaxBackgroundImage.length
			) {
				conf.parallaxBackgroundImage = conf.parallaxBackgroundImage.replace(
					/(?:.*[\\/])?([\w ]+\.\w+)/,
					"media/$1"
				);
			}
			...
			filesToSave.push(
				writeFilePromise(
					path.join(path.dirname(fileName), "configuration.json"),
					JSON.stringify(conf)
				)
			);
			...
			filesToSave.push(
				writeFilePromise(
					path.join(path.dirname(fileName), "css", "custom.css"),
					customCss
				)
			);
			...
			filesToSave.push(
				copyFilePromise(
					path.join(path.resolve(__dirname), "customThemes"),
					path.join(path.dirname(fileName), "css/customThemes")
				)
			);
		...
		//save markdown
		if (md) {
			let mdWithRelativePath = replaceAbsoluteMediaPathWithRelativeInMd(
				md
			);
			filesToSave.push(
				writeFilePromise(
					path.join(
						path.dirname(fileName),
						path.basename(fileName).replace(/\.\w{3,4}/, ".md")
					),
					mdWithRelativePath
				)
			);
				...
				fullfill(
					files
						.filter((file) => {
							return fs
								.statSync(
									path.join(__dirname, options.path, file)
								)
								.isFile();
						})
						.map((fileName) =>
...
function getRemediConfig() {
	return new Promise((fullfill, reject) => {
		fs.readFile(
			path.join(path.resolve(__dirname), "remedi-config.json"),
			"utf8",
			(err, data) => {
				if (err) {
					fullfill({
...
function saveRemediConfig(newConf) {
	return getRemediConfig().then((oldConf) => {
