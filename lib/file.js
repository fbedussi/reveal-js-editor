...
function save(options) {
    ...
    return new Promise((fullfill, reject) => {
        ...
        if (withDependecies) {
			...
			//Copy custom themes
			filesToSave.push(copyFilePromise(path.join(__directory, 'lib', 'customThemes'), path.join(path.dirname(fileName), 'css/customThemes')))
            ...
            filesToSave = filesToSave.concat(
                ['lib/js/classList.js',
                 ...
                 'css'].map(function(file) {
                    return copyFilePromise(path.join(__directory, 'node_modules', 'reveal.js', file), path.join(path.dirname(fileName), file));
                })
            );
