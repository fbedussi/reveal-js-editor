...
import {getPreviewPanelOpen, getPreviewWin, getHtml, getConfiguration, getCustomCss, getCurrentTheme, getCustomThemesList} from '../selectors';
...
class Preview extends React.Component {
	componentWillReceiveProps(nextProps) {
		...
		if (nextProps.previewWin) {
			...
			nextProps.previewWin.webContents.postMessage(message, "*");
		}
	}
	...
	render() {
		return (
			<section className="preview">
				<iframe ref={(f) => this.ifr = f} src="preview.html" width="100%" height="100%" frameBorder="0" scrolling="no"/>
         	</section>
      );
	}
};
