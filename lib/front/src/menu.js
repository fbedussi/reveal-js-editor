...
import {getLabels, getLanguageIso, getCurrentColorScheme, getHtml, getMd, getExportMedia} from './selectors';
import {updateMenuSuccess, openFile, changeLanguage, saveProject, saveMd, saveHtml, copyHtml, showInFolder, openInEditor, menuUpdated} from './actions';
...
function getTemplate(state) {
    ...
    //
    //Get color schemes
    const colorSchemesMenuItems = fs.readdirSync(path.resolve('lib/front/skins'))
        .filter(file => {
            return fs.statSync(path.resolve(path.join('lib/front/skins', file))).isFile();
        })
        .map(fileName => ({
            label: path.basename(fileName, '.css'),
            type: 'radio',
            checked: Boolean(getCurrentColorScheme(state) === 'fileName'),
    ...
    const languageMenuItems = languageList.map(iso => ({
        label: iso,
        type: 'radio',
        checked: Boolean(getLanguageIso(state) === iso),
        click: (menuItem) => {store.dispatch(changeLanguage(menuItem.label));}
    }));
    //
    ...
    const template = [{
        ...
        submenu: [{
            ...
            click: () => {store.dispatch(openFile())}
        }, {
            label: _('Save presentation...'),
            accelerator: 'CmdOrCtrl+S',
            click: () => {
                let state = store.getState();
                store.dispatch(saveProject(getHtml(state), getMd(state), getExportMedia(state)))
            }
        }, {
            label: _('Save markdown only...'),
            click: () => {store.dispatch(saveMd())}
        }, {
            label: _('Save HTML only...'),
            click: () => {store.dispatch(saveHtml())}
        }, {
            label: _('Copy HTML to clipboard'),
            click: () => {store.dispatch(copyHtml())}
        }, {
            label: _('Show in file system'),
            enabled: Boolean(state.main.currentFileName.length),
            //click: () => shell.showItemInFolder(file.getCurrentFile())
            click: () => {store.dispatch(showInFolder())}
        }, {
            label: _('Open in default editor'),
            enabled: Boolean(state.main.currentFileName.length),
            //click: () => shell.openItem(file.getCurrentFile())
            click: () => {store.dispatch(openInEditor())}
...
function dispatchChangeLanguage(menuItem) {
    store.dispatch(changeLanguage(menuItem.label));
}
...
export function menuUpdate(state) {
    
    const menu = Menu.buildFromTemplate(getTemplate(state));
    ...
    store.dispatch(updateMenuSuccess());
}

export function menuInit(s) {
    store = s;
    menuUpdate(store.getState());
}
