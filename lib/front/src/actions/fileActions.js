...
import {getHtml, getMd, getExportMedia, getCurrentFileName, getConfiguration, getRecentFiles,
	getCurrentTheme, getCustomCss, isCustomTheme, getNotificationDuration} from '../selectors';
import {mdChanged, mdLoaded, editorPosChanged, setConfiguration, setCustomCss, setTheme} from './actions';
...
export function openFile(filePath) {
    return function (dispatch, getState) {
        ...
        openFile.then(([{remedi}, {configuration}, {customCss}, {fileName, content}]) => {
                ...
                var state = getState();
                var recentFiles = getRecentFiles(state);
...
export function saveProject(saveThisProject = false) {
    return function (dispatch, getState) {
		...
		if (saveThisProject) {
			options.fileName = getCurrentFileName(state).replace('.md', '.html');
		}
