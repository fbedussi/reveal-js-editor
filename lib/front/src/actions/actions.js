...
import {getHtml, getMd, getExportMedia, getEditorData, getCurrentLanguage} from '../selectors';
...
export function init() {
	return function (dispatch, getState) {
		mainProcess.getThemesList()
			.then((colorSchemesList) => {
				dispatch({type: 'UPDATE_COLOR_SCHEMES_LIST', colorSchemesList})
			})
		;

		mainProcess.getLanguagesList()
			.then((languagesList) => {
				dispatch({type: 'UPDATE_LANGUAGES_LIST', languagesList})
			})
		;

		dispatch(changeLanguage(getCurrentLanguage(getState())));
	}
}
