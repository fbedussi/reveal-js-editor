...
function insert(content, pattern) {
    ...
    history.push(markdownViewEl.value);
...
markdownViewEl.addEventListener('keyup', (event) => {
    if (event.key === 'z' && event.ctrlKey) {
        setMdContent(history.getPrev());
        return;
    }
    
    if (event.key === 'y' && event.ctrlKey) {
        setMdContent(history.getNext());
        return;
    }
    
    const content = event.target.value;
    console.log('content: ', content);
    console.log('current content: ', history.getCurrent());
    if (content !== history.getCurrent()) {
