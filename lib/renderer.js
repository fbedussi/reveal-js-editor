...
function insertAtCursor(value) {
   ...
   markdownViewEl.selectionStart = markdownViewEl.selectionEnd = startPos + value.length; 
}
...
function getCurrentEditLine() {
    ...
    for (start = caretPos - 1; start >= 0 && markdownViewEl.value[start] != "\n"; --start);
...
function clear(value) {
    ...
    if (line.text.match(re)) {
        var newLine = line.text.replace(re, '');
        markdownViewEl.value = markdownViewEl.value.substring(0, line.start) + newLine + markdownViewEl.value.substring(line.end, markdownViewEl.value.length);
        markdownViewEl.selectionStart = markdownViewEl.selectionEnd = line.start + newLine.length;
    }
